    ...
    return (
        <div className={`${fixed && "fixed bottom-0 z-10 w-full lg:w-auto lg:h-full lg:left-0"} flex justify-center items-center`}>
            <ul className="p-2 w-8/12 mb-8 bg-curious-blue-400 flex gap-2 rounded-2xl text-white md:w-2/5 lg:flex-col lg:w-auto lg:ml-16 lg:mb-0">
                {
                    items.map(item => (
                        <Link key={item.path} target={item.target} href={item.path} className={`item-navbar relative bg-curious-blue-400 w-1/3 h-[55px] rounded-lg flex flex-col gap-0.5 items-center justify-center hover:bg-curious-blue-500 transition-colors duration-300 ${item.path === path && "bg-curious-blue-500"} md:h-[75px] lg:w-12 lg:h-12`}>
